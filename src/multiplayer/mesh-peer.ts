...
export interface MeshPeerParams {
  /** The signed-in agent, for reading the peer's repo and writing this player's. */
...
export class MeshPeer {
  private readonly agent: AtprotoRepoLike;
  ...
  private async poll(): Promise<void> {
    ...
    try {
      ...
      const values = res.data.records.map((r) => r.value as unknown);
