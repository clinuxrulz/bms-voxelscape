...
export interface MultiplayerParams {
  /** The signed-in agent, or undefined while anonymous (never captured, always asked). */
  getAgent: () => AtprotoRepoLike | undefined;
...
export class MultiplayerController {
  private readonly getAgent: () => AtprotoRepoLike | undefined;
   ...
   * discovery ticking on their own timers. Requires a signed-in agent.
   */
  async start(): Promise<string> {
    ...
    await this.publishPresence(agent, did, now);
  ...
  private async fetchPresence(did: string): Promise<PresenceRecord | null> {
    ...
    try {
      ...
      const value = res.data.value as unknown;
  ...
  private openPeer(did: string): void {
    ...
    if (agent === undefined || selfDid === null) {
      return;
    }
    let opened = false;
